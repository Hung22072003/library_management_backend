...
class TransactionRepositoryImplement implements TransactionRepositoryInterface
{
    ...
    public function create(array $data)
    {
        return Transaction::create([
            ...
            Transaction::PAYMENT_EXPIRED_AT => Carbon::now()->addDays(2)->format('Y-m-d'),
        ]);
    }
    ...
    public function getOverdueTransactions()
    {
        return Transaction::where('payment_status', '!=', 'success')
            ->where('payment_expired_at', '<', today())
            ->get();
    }

    public function getTotalAmountSuccess()
    {
        return Transaction::where('payment_status', 'success')->sum('amount');
    }
}
